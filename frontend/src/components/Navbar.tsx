...
import { Search, Heart, MessageCircle, User, Menu, X, Bell, ChevronLeft, ChevronRight } from "lucide-react";
...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    try {
      const decoded: { name: string } = jwtDecode(credentialResponse.credential); // Decode Google JWT
      const response = await fetch("http://localhost:5000/google-login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: decoded.name, // Use real name from Google
